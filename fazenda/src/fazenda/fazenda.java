...
public class fazenda {
	...
	public static void main(String[] args) {
		...
		}*/ //tentei colocar o som dos bixos mas a biblioteca n funcionou aqui :(
	...
	static void MenuPrincipal() {
		...
		do { //loop
			try {
				...
				System.out.println("3 - Açoes");
				System.out.println("4 - Sair");
				opcao = scanneer.nextInt();
		
				//verifica opcao, chama o respectivo metodo ou encerra o programa
	...
	static void Acoes() {
		...
		try {
			...
			//verifica opcao, inicia o if de acordo com a opção
	...
	static void MenuVerAnimais() {
		...
		try {
			...
			//verifica opcao, faz o if e executa o metodo
			if(op == 1 || op ==2) {
				if(op==1) {
					System.out.println("1 - Galinha\n 2 - Pato");
	...
	static void MostraMorcego() {
		for(Morcego x : Controle.listaDeMorcegos ) {
			System.out.println("Murça numero: "+ (Controle.listaDeMorcegos.indexOf(x)+(1)));
			System.out.println("Idade:" + x.getIdade());
			System.out.println("Tamanho: " + x.getAltura());
			System.out.println("Cor do pelo :" + x.getCorDoPelo());
			System.out.println();
	...
	static void Animal(int escolha) {
		//recolhe dados de todo animal
		Scanner scanner = new Scanner(System.in);
		Double idade=0.0;
		Double tamanho=0.0;
		
		System.out.println("Qual a idade do animal?\nRepresente um ano como 1,0 e 8 meses como 0,8");
		idade = scanner.nextDouble(); // ex 1,7
		System.out.println("Qual o tamanho dele? \nRepresente um metro como 1,0");
		tamanho = scanner.nextDouble(); //precisa reprentar com , virgula ex 1,7
		
		// de acordo com a escolha feita, que é trazida com o metodo
		// Cria um mamifero ou uma ave
			if(escolha == 1) {
	...
	static void Mamiferos(Double idade, Double tamanho) {
		...
		try{ //verifica escolha e cria uma vaca ou um morcego
			if(escolha == 1 || escolha == 2) {
				if(escolha == 1) {
					...
					System.out.println("opção invalida"); // dps ajustar 
			}
		}
	...
	static void Aves(Double idade, Double tamanho) {
		...
		escolha = opcaoEscolhida2.nextInt();
		try{ // verifica escolha e cria galinha ou pato
			if(escolha == 1 || escolha == 2) {
				if(escolha == 1) {
